...
def save_preds(y_pred, csv_path, out_path):
    ...
    subject_lst = pd.read_csv(csv_path)['subject_id']
    study_lst = pd.read_csv(csv_path)['study_id']

    full_reports = pd.read_csv(csv_path)['report']

    df['Report Impression'] = reports.tolist()
    df['subject_id'] = subject_lst.tolist()
    df['study_id'] = study_lst.tolist()
    df['report'] = full_reports.tolist()

    new_cols = ['subject_id'] + ['study_id'] + ['report'] + ['Report Impression'] + CONDITIONS
    df = df[new_cols]
